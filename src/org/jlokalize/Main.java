...
public class Main {
    ...
    /** Path to the user directory, we will store the options and the language files and the logs there */
     ...
     * yet existing. The setup of the logger is quite standard. We will append all
    ...
    /**
     * Registers available dictionaries. If none is available, sets the option to
     * use the spell checker to false.
     */
    ...
    private static void newConfiguration(Resource resource) throws IOException {
        ...
        for (Resource in : languages) {
            ...
            ResourceUtils.copyText(in, out);
        }
    }

    /**
     * From all available languages (properties files in JLokalize.config/lang)
     * either choose one via a selection dialog or take an options entry and load
     * a PropertyWithStats object from it. English is set as master. The default
     * locale of the gui is also updated. The dialog is displayed if the preferred
     * language equals "none".
     *
    ...
    public static boolean setupLanguage(String prefLanguage) throws IOException {
        ...
        for (Resource resource : resources) {
            ...
            Locale locale = I18nUtils.createLocaleFromFileName(id);
            String name = CommonUtils.capitalize(locale.getDisplayName());
        ...
        if ("none".equals(prefLanguage)) {
            ...
            List<String> names = CommonUtils.asSortedList(languages.keySet());
            // i18n the dialog
            ...
            String name = ListSelectDialog.createAndRun(null, title, buttonText, names);
            if (name == null) {
